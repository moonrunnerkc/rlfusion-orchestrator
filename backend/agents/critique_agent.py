...
class CritiqueAgent:
    ...
    def plan(self, state: PipelineState) -> PipelineState:
        ...
        logger.debug(
            "[%s] Planning critique: response_present=%s", self._NAME, has_response
        )
        ...
        log_episode_to_replay_buffer(
            {
                ...
                "proactive_suggestions": critique_result.get(
    ...
    def reflect(self, state: PipelineState) -> PipelineState:
        ...
        if reward < 0.0 or reward > 1.0:
            logger.warning(
                "[%s] Reward out of bounds: %.2f (clamping to [0, 1])",
                self._NAME,
                reward,
            )
        if reward < 0.3:
            logger.warning(
                "[%s] Low reward (%.2f) for query: %s...",
                self._NAME,
                reward,
                state.get("query", "")[:50],
            )
        ...
        logger.debug(
            "[%s] Critique complete: reward=%.2f, suggestions=%d",
            self._NAME,
            reward,
            len(suggestions),
        )
