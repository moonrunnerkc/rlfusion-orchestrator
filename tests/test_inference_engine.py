...
class TestInferenceEngineInit:
    ...
    def test_engine_respects_env_override(self):
        ...
        with patch.dict(
            os.environ,
            {"INFERENCE_ENGINE": "vllm", "INFERENCE_BASE_URL": "http://vllm:8000"},
        ):
            engine = InferenceEngine()
...
class TestInferenceEngineOllama:
    ...
    def test_generate_calls_ollama_client(self):
        ...
        with patch.dict(os.environ, {"INFERENCE_ENGINE": "ollama"}):
            ...
            with patch(
                "backend.core.model_router.InferenceEngine._ollama_generate",
                wraps=engine._ollama_generate,
            ):
...
class TestInferenceEngineHealthCheck:
    ...
    def test_ollama_health_check_success(self):
        ...
        with patch.dict(
            os.environ,
            {"INFERENCE_ENGINE": "ollama", "INFERENCE_MODEL": "dolphin-llama3:8b"},
        ):
            engine = InferenceEngine()

            mock_response = MagicMock()
            mock_response.json.return_value = {
                "models": [{"name": "dolphin-llama3:8b"}]
            }
            mock_response.raise_for_status = MagicMock()
